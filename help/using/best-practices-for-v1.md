...
[!DNL Adobe Experience Manager] 데스크탑 앱은 DAM(디지털 자산 관리) 솔루션을 데스크탑과 연결하여 데스크탑에서 바로 AEM Web UI에서 사용할 수 있는 파일을 열 수 있습니다. 데스크탑에서 자산을 저장한 경우 적절한 위치에서 AEM으로 업로드됩니다.
...
>[!NOTE]
>
>이 문서를 읽기 전에 전체 내용을 검토할 수 있습니다. [AEM 및 Creative Cloud 통합 우수 사례](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/aem-cc-integration-best-practices) 를 참조하십시오.
...
* **데스크탑에서 AEM Assets Web UI에서 파일 열기**. 웹 UI에서 데스크탑의 에셋을 표시하거나(Finder, Explorer) 데스크탑 애플리케이션을 사용하여 에셋을 열 수 있습니다.
...
* **데스크탑에서 자산 액세스**. 마운트된 네트워크 공유에서 상황별 메뉴는 [!UICONTROL More Info] 대화 상자(더 큰 미리 보기, 주요 메타데이터) 및 AEM UI에서 에셋을 여는 기능.

* **크고 계층적인 폴더를 일괄적으로 업로드**. 를 사용하는 경우 **만들기** > **폴더 업로드** AEM UI의 옵션을 사용하여 에셋을 업로드하면 AEM 데스크탑 앱에서 선택한 폴더 계층 구조를 배경에 있는 AEM에 업로드합니다. 업로드 진행률은 데스크탑 앱의 전용 UI를 통해 모니터링됩니다.
...
* 한 AEM 인스턴스에서 다른 AEM 인스턴스로 대량의 데이터를 마이그레이션하는 데 데스크탑 앱을 사용하지 마십시오. 다음을 참조하십시오. [마이그레이션 안내서](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/assets-migration-guide) 자산 마이그레이션을 계획 및 실행합니다. 반면 데스크탑 앱은 [벌크 업로드 지원](use-app-v1.md#bulkupload) 에서 처음으로 많은 에셋 [!DNL Adobe Experience Manager].
...
AEM Assets 네트워크 구성에 대한 모범 사례를 이해하려면 [자산을 대량 마이그레이션하는 방법](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/assets-migration-guide) 문서. 사용자를 위한 AEM 데스크탑 앱 환경을 최적화하는 데 도움이 되는 몇 가지 중요한 측면은 다음과 같습니다.

* **올바르게 구성된 Dispatcher 사용**. 추가 보안을 위해 AEM Dispatcher을 사용하고 이 가 [Dispatcher을 통해 AEM에 AEM 데스크탑 앱 연결](install-configure-app-v1.md#connect-to-an-aem-instance-behind-a-dispatcher)
...
AEM Assets 서버를 성능에 맞게 최적화하는 방법을 이해하려면 [AEM Assets 성능 조정 안내서](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/performance-tuning-guidelines). AEM 데스크탑 앱의 서버 성능에 대한 몇 가지 중요한 측면은 자산 업로드에 잘 수행되도록 워크플로 구성을 최적화하는 것입니다.

* **더 많은 수행자 에셋 업로드**. 구성 [AEM Asset 업데이트 워크플로 모델이 임시적이 됨](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/performance-tuning-guidelines).
