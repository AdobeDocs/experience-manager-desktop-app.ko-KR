...
**간소화된 에셋 상태 창**. 상태 창이 단순해졌습니다. 다음 [!UICONTROL Uploads] 이제 탭에 개별 에셋과 폴더 또는 벌크 업로드가 모두 표시됩니다. 이전 일괄 업로드 탭이 제거되었습니다.
...
**연결 손실 처리 능력 향상**. 연결 손실이 있는 경우 벌크 업로드가 일시 중지되며 사용자가 나중에 다시 시작할 수 있습니다. A [!UICONTROL Retry] 개별 파일의 실패한 업로드를 다시 시도하는 데 옵션을 사용할 수 있습니다.
...
* **버전 1.8**: 사용자의 캐시 크기 제어 향상, Windows에서 SAML/SSO에 대한 로그인 환경 개선, 지원 `.pac` Mac의 네트워크 프록시 및 고객이 보고한 문제
...
[!DNL Adobe Experience Manager] 데스크탑 앱 버전 1.9.1은 패치 릴리스입니다. 자산 체크아웃과 관련된 주요 고객 문제를 해결하도록 설계되었습니다. 네트워크 공유에서 로컬 디렉터리로 파일을 복사하는 주소입니다.
...
다음을 통해 큰 폴더 계층 구조 업로드 [!UICONTROL Create] > [!UICONTROL Upload Folder] AEM 웹 UI에서 이 대화 상자를 트리거합니다. 데스크탑 앱 컨텍스트 메뉴에서 Finder 또는 Explorer에서 &quot;Assets 붙여넣기&quot;를 복사하여 선택할 때도 마찬가지입니다.
...
* 네트워크:
   ...
   * (Mac) 를 사용하여 AEM에 연결하기 위한 지원을 추가합니다. `.pac` 프록시 구성(CQ-4233430).
...
**macOS X 및 Windows용 버전 1.5.0.x:** AEM 데스크탑 앱의 이번 릴리스는 다음과 같은 이점을 제공합니다.
...
이유: `/Volumes` 폴더에 더 이상 관리 권한이 필요하지 않습니다. 이 변경 사항은 1.5.1에서 되돌려졌습니다. 또한 이 변경 사항으로 macOS 사용자 간에 AEM의 에셋을 배치한 InDesign 문서를 공유할 수 있습니다.

### 프로토콜 변경(v1.3 이후) {#protocol-change-since}

* MACOS X
   * OS X 데스크톱 통합을 위한 로컬 네트워크 드라이브 프로토콜이 WebDAV에서 SMB1로 변경되었습니다.
   * 데스크탑 앱과 함께 마운트된 AEM 저장소는 로 표시됩니다. `smb` WebDAV 드라이브 대신 Finder의 네트워크 드라이브.
* Windows:
...
>[!NOTE]
>
>macOS X에서 로컬 네트워크 드라이브 프로토콜을 WebDAV에서 SMB1로 변경하면 저장소의 동일한 에셋에 대한 다른 로컬 경로가 생성됩니다. 이 변경 사항은 &quot;배치&quot; 명령을 통해 Adobe Creative Cloud 애플리케이션에 배치된 파일에 대한 링크에 영향을 줄 수 있습니다. 다음을 참조하십시오. [AEM 데스크탑 앱 사용](use-app-v1.md) 추가 정보.
...
* 두 번째 단계에서는 사전 정의된 지연(예: 30초) 후 데스크탑 앱에서 업데이트된 파일을 AEM 서버에 업로드합니다. 이 작업은 백그라운드에서 수행됩니다. 사용 **백그라운드 파일 동기화 상태 표시** 업로드 작업의 상태를 보는 옵션입니다.

## 중요 공지 {#important-notices}

**폴더를 업로드합니다.** Adobe은 새 폴더 업로드 기능을 사용하여 더 큰 계층 구조의 폴더를 AEM에 업로드할 것을 권장합니다. 이 방법은 Finder/Explorer 수준에서 마운트된 AEM 저장소로 복사/드래그 앤 드롭하는 것과 대조적으로 권장됩니다. 폴더 업로드 기능을 사용하는 경우 데스크탑 앱은 AEM과 직접 통신하므로 전체 프로세스를 훨씬 잘 제어할 수 있습니다.

**AEM 세션을 사용할 수 있도록 유지합니다.** AEM 데스크톱 앱은 AEM Assets 서버에 열려 있는 세션에 따라 제대로 작동하는지 확인합니다. 일별 사용자는 하루 종일 AEM Assets을 마운트 해제하여 로그아웃한 후 오전에 다시 마운트하여 로그인 및 네트워크 공유 기능을 유지해야 합니다.
...
**새 AEM 서버에 연결할 때 캐시를 정리합니다.** 데스크탑 앱이 동일한 URL을 사용하여 다른 AEM 서버에 연결하는 경우 캐시가 자동으로 지워지지 않습니다. 올바른 작업을 위해 캐시를 수동으로 지웁니다. 이 프로세스는 동일한 URL에서 실행되는 동안 AEM 설치를 바꿀 수 있는 경우 일반적으로 테스트에서 발생합니다(CQ-4216982)
...
* Finder 통합:
   * 큰 파일을 드래그 앤 드롭할 때 파일이 백그라운드에서 전송되는 동안 Finder가 시간 초과될 수 있습니다. 이 지연으로 인해 `Error - 36`. 해결 방법은 자산을 드래그 앤 드롭하거나 다시 여는 것입니다. (CQ-4219628)
...
* Windows:
   ...
   * 앱을 선택하면 앱이 충돌할 수 있습니다. [!UICONTROL Retry] 실패한 업로드에 대한 옵션입니다. 이 충돌은 연결이 끊겼을 때 일괄 업로드를 재개한 후 특정 상황에서 발생할 수 있습니다. (CQ-4251884) (Win)
